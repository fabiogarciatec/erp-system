...
-- Criar tabelas básicas com campo document maior
...
CREATE TABLE profiles (
    ...
    is_active BOOLEAN DEFAULT true, -- Adicionado campo is_active
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
-- Criar a função com tratamento de erro melhorado
...
BEGIN
    ...
    IF length(p_company_document) > 50 THEN
        RETURN jsonb_build_object(
            'success', false,
            'error', 'Documento muito longo. Máximo de 50 caracteres.',
            'error_detail', 'DOCUMENT_TOO_LONG'
        );
    END IF;
    ...
    -- Criar perfil (agora com is_active)
    INSERT INTO profiles (user_id, email, role_id, company_id, is_active)
    VALUES (p_user_id, p_email, v_role_id, v_company_id, true);
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in create_user_complete: %, SQLSTATE: %', SQLERRM, SQLSTATE;
        RETURN jsonb_build_object(
            'success', false,
            'error', SQLERRM,
            'error_detail', SQLSTATE,
            'debug_info', jsonb_build_object(
                'company_document_length', length(p_company_document),
                'company_document', p_company_document
            )
        );
END;
$$;
